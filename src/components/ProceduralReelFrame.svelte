<script lang="ts">
	...
	const launchEnergy = $derived(
		launchElapsed < 0.62 ? Math.max(0, 1 - launchElapsed / 0.62) : 0,
	);
	...
	const bottomCloudY = $derived(
		grid.y + grid.height + Math.max(18, grid.height * 0.04) * 0.78,
	);
	...
	const drawBackground = (g: import('pixi.js').Graphics) => {
		...
		chamferPath(g, grid.x - border, grid.y - border, grid.width + border * 2, grid.height + border * 2, 18).fill({
			color: palette.rail,
			alpha: 1,
		});
	...
	const drawOverlay = (g: import('pixi.js').Graphics) => {
		...
		chamferPath(g, centerX - plaqueWidth / 2, frameY - plaqueHeight * 0.28, plaqueWidth, plaqueHeight, 10).fill({
			color: palette.frameShadow,
			alpha: 1,
		});
		chamferPath(g, centerX - plaqueWidth / 2, frameY - plaqueHeight * 0.28, plaqueWidth, plaqueHeight, 10).stroke({
			width: 3,
			color: palette.frameLight,
			alpha: 0.95,
		});
	...
	const drawBottomLaunch = (g: import('pixi.js').Graphics) => {
		...
		g.roundRect(grid.x - border * 0.8, y - height * 0.5, grid.width + border * 1.6, height, 12).fill({
			color: palette.glow,
			alpha: 0.48,
		});
		g.roundRect(grid.x - border * 0.35, y - height * 0.26, grid.width + border * 0.7, height * 0.52, 8).stroke({
			width: 3,
			color: palette.highlight,
			alpha: 0.86,
		});
