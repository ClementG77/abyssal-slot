...
logic, no RNG on the client. The Eye "build-up and reveal" is a _replay of a
predetermined outcome_.
...
- **Scatters land organically (Sweet-Bonanza style) in base/ante.** They appear on the
  ...
  **persist** (scatters never explode). The trigger fires when **≥4 are on the board at the
  ...
  _(Buy modes show their scatters on the trigger reveal instead.)_
...
_normal win, big win, win cap (max win), loss (zero payout), bonus trigger._
...
- **Buy modes have no separate "bonus trigger"** — every book _is_ a bonus round, so
  any id qualifies.
